...
- ``papss`` expects pupil dilation data in a *pupil* column. 
- ``papss`` expects the time variable in a *time* column.
...
- The time variable should contain the same values for each factor level! I.e., the same time-series
should be available for each condition.
...
- *maxiter_outer*: Maximum number of iterations for outer estimator (spike penalization). Should be increased if ``papss`` does not converge (and these issues do not depend on ``t_max``).
...
demand_extract <- extract_demand_for_fact(aggr_dat,
                                          ...
                                          500/20, # expand_by/sample_length arguments for pupil_solve()
                                          factor_id = "subject") 
...
that they match the model that we would expect:
...
bootstrap <- papss::bootstrap_papss_standard_error(solvedPupil$coef,
                                                   aggr_dat$pupil,
                                                   solvedPupil$setup,
                                                   N=10, # This should be higher in practice!
...
and that all subjects were very similar in how they experienced demand.
...
In that case it does not become desirable to fit a shared model across all subjects
because the demand solutions for different subjects will likely differ in smoothness as well.
...
subjects' demand trajectories for different  will likely not reveal a meaningful shared trend.

- In that case, ``pupil_slove()`` is better called on **condition averages** instead of subject averages
(as recommended by Hoeks & Levelt, 1993). In that case, subjects' demand trajectories should still be estimated
